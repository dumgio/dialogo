...
import { FASI, TEMI, MAIL, CONTATTI, TIPI } from './content.js';
import { DURATE_PER_TIPO, DURATA_MIN, DURATA_MAX, VOCI_AUTOVALUTAZIONE } from './logic.js';
...
export function editorDurata(p, fasi = FASI) {
  return `
  <div class="scheda editor">
    ...
    <p class="nota" style="margin-top:14px">Le fasi seguono le proporzioni consigliate. Se vuoi, cambia i minuti di ogni fase: la durata totale si aggiorna da sola.</p>
    ${fasi.map((f, i) => `
...
export function controlliDomande(cerca, filtro, conFiltro = true) {
  return `
  <label class="campo" for="cerca">Cerca una domanda o un argomento</label>
  <input type="search" id="cerca" placeholder="${conFiltro ? 'Per esempio: amicizia, libertà, scuola' : 'Per esempio: fatica, attenzione, limite'}" value="${esc(cerca)}" autocomplete="off">
  ${conFiltro ? `<div id="chips-pub" style="margin-top:10px">${chipsPubblico(filtro)}</div>` : ''}`;
}

// Un menu a tendina per argomento. Con «seleziona» ogni domanda è un pulsante, altrimenti solo testo.
export function accordionDomande({ domande, aperto, cerca, seleziona, temi = TEMI }) {
...
export function tabellaDurate(tipo = 'filosofico') {
  const righe = TIPI[tipo].fasi.map((f, i) => `<tr><td>${i + 1} · ${esc(f.titolo)}</td>${[60, 90, 120].map((d) => `<td>${DURATE_PER_TIPO[tipo][d][i]}</td>`).join('')}</tr>`).join('');
...
export function blocco(b, domandeHtml) {
  switch (b.t) {
    ...
    case 'durate': return tabellaDurate(b.tipo || 'filosofico');
    default: return '';
  }
}
...
export function vistaScheda(s) {
  ...
  ${(s.voci || VOCI_AUTOVALUTAZIONE).map((v, i) => `<div class="voto-riga"><p>${esc(v)}</p><div class="voti">${[1, 2, 3, 4, 5].map((n) =>
